...
class RegisterDoctor extends Component {
    ...
    handleSubmit(event) {
        ...
        .then(response =>{
            ...
            //    this.changerLoadHeard();
               this.getRedux();
                notification.success({
                    ...
                    description: "Thank you! You have successfully registered. Please create a clinic to schedule appointments!",
                });  
            ...
            notification.error({
                message: 'Booking Clinic',
                description: error.message || "I'm sorry, but the registration has failed !"
            });
        })
    ...
    validateTokenCode = (tokenCode) =>{
        if(tokenCode.length === 0){
            return {
                ...
                errorMsg: `You need to enter the code provided by the admin !`
            }
        }else {
            return {
    ...
    validateFullName = (fullName) =>{
        if(fullName.length < NAME_MIN_LENGTH){
            return {
                validationStatus: 'error',
                errorMsg: `The full name is too short! Please enter a longer than ( ${NAME_MIN_LENGTH} )  characters!`
            }
        } else if (fullName.length > NAME_MAX_LENGTH) {
            return {
                validationStatus: 'error',
                errorMsg: `The full name is too long! Please enter a shorter than ( ${NAME_MAX_LENGTH} )  characters!`
            }
        } else {
            return {
                validateStatus: 'success',
    ...
    validateMobile = (mobile) =>{
        if(mobile.length < MOBILE_MIN_LENGTH) {
            return {
                ...
                errorMsg: `The phone number is too short! Please enter a longer than ( "${MOBILE_MIN_LENGTH}" ) characters!`
            }
        } else if (mobile.length > MOBILE_MAX_LENGTH) {
            return {
                validationStatus: 'error',
                errorMsg: `The phone number is too longer! Please enter a shorter than ( ${MOBILE_MAX_LENGTH} )  characters!`
            }
        } else {
            return {
                validateStatus: 'success',
    ...
    validateAddress = (address) =>{
        if (address.length > ADDRESS_MAX_LENGTH) {
            return {
                validationStatus: 'error',
                errorMsg: `The address is too long! Please enter a shorter than ( ${ADDRESS_MAX_LENGTH} )  characters!`
            }
        } else {
            return {
                validateStatus: 'success',
    ...
    validateAbout = (about) =>{
        if(about.length === 0){
            return {
                validationStatus: 'error',
                errorMsg: `You need to enter basic information!`
            }
        }else {
            return {
                validateStatus: 'success',
